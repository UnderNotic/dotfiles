...
checkVersionFeasibilityWithVPN()
{
  ...
  if [ -f "${NEWTEMP}/ACManifestNVM.xml" ] && [ -f ${VPNMANIFEST} ]; then
      ...
      NVMCURRVERSION=$(awk -F"\"" '/file version/ { print $2 }' ${NEWTEMP}/ACManifestNVM.xml)
...
checkVersionFeasibilityWithNVM()
{
  ...
  if [ "$STANDALONE_FLAG" = true ] && [ -f "${NEWTEMP}/ACManifestNVM.xml" ] && [ -f ${NVMMANIFEST} ]; then
...
LOGFNAME=`date "+anyconnect-linux64-4.9.01095-nvm-install-%H%M%S%d%m%Y.log"`
