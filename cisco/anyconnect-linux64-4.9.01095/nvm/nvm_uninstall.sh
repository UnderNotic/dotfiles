...
killNVMAgent()
{
  if [ ! "x${NVMPROC}" = "x" ] ; then
      ...
      kill -KILL ${NVMPROC} >> ${UNINSTALLLOG} 2>&1
  fi
}
...
echo "rm -f ${PLUGINDIR}/libacnvmctrl.so" >> ${UNINSTALLLOG}
rm -f ${PLUGINDIR}/libacnvmctrl.so >> ${UNINSTALLLOG} 2>&1
