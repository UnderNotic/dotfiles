...
checkVersionFeasibilityWithStandaloneNVM()
{
  ...
  if [ -f "${NEWTEMP}/ACManifestVPN.xml" ] && [ -f ${NVMMANIFEST} ]; then
      VPNVERSION=$(awk -F"\"" '/file version/ { print $2 }' ${NEWTEMP}/ACManifestVPN.xml)
...
checkVersionFeasibilityWithVPN()
{
  ...
  if [ -f "${NEWTEMP}/ACManifestVPN.xml" ] && [ -f ${VPNMANIFEST} ]; then
      VPNPREVVERSION=$(awk -F"\"" '/file version/ { print $2 }' ${VPNMANIFEST})
      VPNCURRVERSION=$(awk -F"\"" '/file version/ { print $2 }' ${NEWTEMP}/ACManifestVPN.xml)
...
LOGFNAME=`date "+anyconnect-linux64-4.9.01095-core-vpn-webdeploy-k9-%H%M%S%d%m%Y.log"`
...
${INSTALL} -o root -m 755 ${NEWTEMP}/load_tun.sh ${BINDIR} || exit 1
...
${INSTALL} -o root -m 755 ${NEWTEMP}/libacruntime.so ${LIBDIR} || exit 1
...
${INSTALL} -o root -m 755 ${NEWTEMP}/cfom.so ${LIBDIR} || exit 1

echo "Installing "${NEWTEMP}/libaccurl.so.4.5.0 >> /tmp/${LOGFNAME}
${INSTALL} -o root -m 755 ${NEWTEMP}/libaccurl.so.4.5.0 ${LIBDIR} || exit 1

echo "Creating symlink "${NEWTEMP}/libaccurl.so.4 >> /tmp/${LOGFNAME}
ln -s ${LIBDIR}/libaccurl.so.4.5.0 ${LIBDIR}/libaccurl.so.4 || exit 1
...
if [ -f "${NEWTEMP}/libacwebhelper.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libacwebhelper.so ${PLUGINDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_date_time.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_date_time.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_filesystem.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_filesystem.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_system.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_system.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_thread.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_thread.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_signals.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_signals.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/libboost_chrono.so" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/libboost_chrono.so ${LIBDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/acwebhelper" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/acwebhelper ${BINDIR} || exit 1
else
...
if [ -f "${NEWTEMP}/manifesttool_vpn" ]; then
    ...
    ${INSTALL} -o root -m 755 ${NEWTEMP}/manifesttool_vpn ${BINDIR} || exit 1
...
if [ -z "${TESTINIT##*"systemd"*}" ]; then
  ...
  systemctl start ${INIT} >> /tmp/${LOGFNAME} 2>&1 || exit 1
elif [ "x${INITD}" != "x" ]; then
